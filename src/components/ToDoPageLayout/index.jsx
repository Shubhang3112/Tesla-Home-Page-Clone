...
const ToDoPageLayout = () => {
  ...
  const addTaskToArray = (task, taskArray, setTaskArray, setTask) => {
    setTask("");
    if (task.length === 0) return;
    for (let i = 0; i < taskArray.length; i++) {
      if (taskArray[i] === task) return;
    }
    setTaskArray([...taskArray, task]);
  };
  const deleteTaskFromArray = (taskArray, setTaskArray, tasks) => {
    setTaskArray(taskArray.filter((a) => a !== tasks));
  };
  const Search = (setFilteredData, data, value) => {
    setTask(value);
    if (value.length === 0) {
  ...
  const PlaceholderData = async () => {
    try {
      const response = await makeRequest("get", API_ENDPOINTS.placeholder);
  ...
  // const [sumPlus, setSumPlus] = useState(20);

  // const updateArray = useMemo(() => {
  //   return sum + sumPlus;
  // }, [sum, sumPlus]);

  // const takecounts = useCallback(() => {}, [sum]);
  var settings = {
    dots: false,
    infinite: false,
    // speed: 500,
  ...
  return (
    <div className="to_do_page">
      <h1>ToDo App</h1>

      <div className="to_do_input_ctn">
        <input
          type="text"
          onChange={(e) => Search(setFilteredData, data, e.target.value)}
          value={task}
        />
        <button
          onClick={() => addTaskToArray(task, taskArray, setTaskArray, setTask)}
        >
          Add Task
        </button>
      </div>
      <div className="task_container">
        {/* <Slider {...settings}>
        {filteredData.map((Inddata,index) => (
          <div key={index}>
          <div className="tasks">
            <p>{Inddata.title}</p>
          </div>
          ...
          {taskArray.map((tasks, index) => (
            <div key={index}>
              <div className="tasks">
                ...
                <button
                  onClick={() =>
                    deleteTaskFromArray(taskArray, setTaskArray, tasks)
                  }
                >
                  <svg
                    fill="#000000"
